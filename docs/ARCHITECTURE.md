...
- **Tags** — generic categories on photos (`file_tags`); Browse supports multi-label **AND** filtering (`/browse/tags?tag=…&person=…&camera=…`) with co-occurring tags, a **people subset**, and a **cameras subset** for the current selection (`GET /api/browse/cooccurring`). With 1–5 labels selected, a **Photos / Venn** toggle shows a hybrid Venn diagram (1 circle, 2–3 circles, 4 ellipses, or 5 petals; layout morphs when labels change; exclusive region counts from `GET /api/browse/venn`; `?view=venn`). Calendar year/month chips remain single-label (multi-chip follow-up).
...
| Browse filters | `GET /api/browse/cooccurring` (tags + people + cameras in AND selection); `GET /api/browse/venn` (exclusive region counts for 1–5 labels) |
